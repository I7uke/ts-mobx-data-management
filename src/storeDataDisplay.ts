...
function validationPaginationNumber(num?: number | null): number {
...
function countMaxPages(params: CountMaxPagesParams): number {
    const totalItems: number = validationPaginationNumber(params.totalItems);
    const itemsOnPage: number = validationPaginationNumber(params.itemsOnPage);
...
function validationCurrentPage(params: ValidationCurrentPageParams): number {
    const maxPages: number = validationPaginationNumber(params.maxPages);

    if(!maxPages) {
        return 0;
    }

    const currentPage: number = validationPaginationNumber(params.currentPage);
...
function getPagination<TItem extends DataSourceItem>(params: GetPaginationParams<TItem>):Pagination<TItem> {
    const totalItems: number = validationPaginationNumber(params.allItemsList.length);
...
export default class StoreDataDisplay<TItem extends DataSourceItem> {
    ...
    private _itemsOnCurrentPage_observable: TItem[];
    private _itemsList:TItem[];
    private _pagination_observable: Pagination<TItem>;

    private _getDataOnCurrentPage(){
    ...
    constructor(init: InitStoreDataDisplay<TItem>) {
        ...
        this._itemsOnCurrentPage_observable = [];
        this._pagination_observable = getEmptyPagination();

        this._itemsList = [];

        makeObservable<this,
            '_itemsOnCurrentPage_observable'
            | '_paginationSelectedData_observable'
            | '_pagination_observable'>(this, {
            _itemsOnCurrentPage_observable: observable.ref,
